...
import fetchSchools from '@salesforce/apex/candidateMessaging.fetchSchoolsForRecruiters';
import fetchInboxContacts from '@salesforce/apex/candidateMessaging.fetchInboxContactsRecruiter';
...
export default class RecruiterMessagingContacts extends LightningElement {
    @track selectedSchool = '';
    @track schoolOptionList = [];
    @track options = [];
    @track showSpinner = true;
    @track inboxContacts = [];
    @track currentUserId = '0038M00000HA3ZXQA1';
    ...
    connectedCallback() {
        getCurrentUserId().then(result => {
            ...
            fetchSchools({ currentUserId: this.currentUserId }).then(result => {
                console.log('school List : ', result);
                ...
                data.forEach(element => {
                    ...
                    if (JSON.parse(element[0]).Id != this.currentUserId) {
                        ...
                        item.push({
                            ...
                            class: className,
                            schoolId: JSON.parse(element[0]).AccountId
                        })
                    }
                });
    ...
    handleChange(event) {
        ...
        this.noContacts = false;
        console.log('School Selected : ' + this.selectedSchool);
        fetchInboxContacts({ relatedSchool: this.selectedSchool }).then(result => {
    ...
    openChat(event) {
        ...
        console.log('Other Person Id : ' + event.currentTarget.dataset.id);
        console.log('Contact name : ' + contactName);
        console.log('Contact Address : ' + contactAddress);
        console.log('Contact Id : ' + candidateId);

        const customEvent = new CustomEvent('openchat', { detail: [event.currentTarget.dataset.id, contactName, contactAddress, candidateSchool, candidateId], bubbles: true });
        this.dispatchEvent(customEvent);
